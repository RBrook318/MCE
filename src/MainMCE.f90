Program MainMCE
  ...
  !***********************************************************************************!
    ...
    real(kind=8) :: num1, num2, hc, random
    real, dimension(:), allocatable :: cloneblock
    integer, dimension(:), allocatable :: clonetype
    ...
    !$omp                    nrmtmp, nrm2tmp, ehrtmp, gridsp, timestrt_loc, trspace, clonetype,     &
    ...
    !$omp                    nclones, clone1, clone2, populations, ctarray, normpfs, random,        &
    !$omp                    range, rescale, i, p, g, clonememflg, e, ovrlphold, cloneblock         )
    ...
    do k=1,reptot,intvl              ! Loop over all repeats.
      ...
      if (cloneflg == 'V1') then
        if (auto_clone == 'NO') then 
            if (mod(tnum-2, clone_block) == 0) then
        ...
        else if (auto_clone == 'RAN') then
            ! Generate random timesteps between 1 and clone_block
            num_events = min(clone_block, clonemax)
            write(6,*) 'num of events ', num_events
            allocate(cloneblock(num_events))
            allocate(clonetype(num_events))
            call RANDOM_NUMBER(random)
            cloneblock(1) = 1 + floor(random * clone_block)
            write(6,*) clonefreq
            ! Generate subsequent timesteps
            do n = 2, size(cloneblock)
                ! Generate a random timestep between clone_freq and clone_block after the last generated timestep
                call RANDOM_NUMBER(random)
                write(6,*) cloneblock(n-1) + clonefreq
                cloneblock(n) = cloneblock(n-1) + clonefreq + floor(random * (clone_block))
            end do
            
            ! Allocate bsetarr based on randomly generated timesteps
            two_to_num_events = int(2 ** num_events)
            call allocbs_alt(bsetarr, two_to_num_events, in_nbf) !allocate(bsetarr(2**num_events, clonefreq))
          else if (auto_clone == 'TOP') then 
            num_events = 1
            write(6,*) 'num of events, here ', num_events
            allocate(cloneblock(num_events))
            allocate(clonetype(num_events))
            cloneblock(1) =0.00
            cloneblock(2) = 0.00
          ...
          call allocbs_alt(bsetarr,2**clonemax,in_nbf)
        end if
      end if
        ...
        if (prop.eq."Y") then     ! Propagation of basis set
          ...
          do while ((time.lt.timeend_loc).and.(x.le.tnum+2)) !timestep loop 
            ...
            if (cloneflg=="V1") then 
              ...
              if (auto_clone=='TOP') then
                call v1ctcloning_check(bsetarr, cloneblock, e, nbf, nclones,x,clonetype,pops)
              end if
              do p = 1, nclones 
                call propstep (bsetarr(p)%bs, dt, dtnext, dtdone, time, genflg, timestrt_loc,x,reps)     ! Takes a single timestep
              end do 
              if (auto_clone.ne.'TOP') then
                call v1cloning_check(bsetarr, cloneblock, e, nbf, nclones,x,clonetype,pops)
              end if
              ...
              if (nclones.gt.1) then 
                ...
                call postprop(bsetarr(1)%bs,nbf,x,y,reps,muq,mup,time,popt,pops,timestrt_loc,timeend_loc,dt,absehr, &
                  absnorm,absnorm2,acf_t,extra,clone,clonenum)
              end if
